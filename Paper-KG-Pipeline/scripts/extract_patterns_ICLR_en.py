...
def load_done_ids(out_path: Path) -> set:
    """
    Read existing jsonl output and collect all processed paper_id.
    Both successful extractions and error records are treated as done,
    so we can finish the full dataset without getting stuck.
    """
    done = set()
...
def main():
    ...
    # Resume: read already processed ids from existing output
    done_ids = load_done_ids(OUT_PATH)
    print(f"[resume] found {len(done_ids)} already processed records in {OUT_PATH}")
    ...
    with open(OUT_PATH, "a", encoding="utf-8") as f:
        for row in tqdm(ds, total=len(ds), desc="Extracting patterns (resume)"):
            ...
            # Stop after writing N new items (instead of ds head N)
            if newly_written >= N:
                break
            ...
            if not ok:
                ...
                # Mark as done to avoid infinite retries
                done_ids.add(paper_id)
                newly_written += 1

    print(f"[done] newly_written={newly_written}, skipped(existing/no-id)={skipped}, out={OUT_PATH}")
