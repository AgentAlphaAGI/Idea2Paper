...
class AgentManager:
    def __init__(self, task_desc: str, cfg: Any, workspace_dir: Path):
        ...
        self.stage_policy = self._get_stage_policy()
        stage2_hf_new = self.stage_policy["stage2_hf_new_datasets"]
        stage3_hf_total = self.stage_policy["stage3_hf_total_datasets"]

        def _format_goals(lines: List[str]) -> str:
            return "\n                " + "\n                ".join(lines)
        ...
        if stage2_hf_new > 0:
            stage2_goals.append(
                f"- Introduce {stage2_hf_new} more new datasets from HuggingFace test the model. Try very hard to think what Huggingface datasets can be used here for testing."
            )
        stage3_goals = [
        ...
        if stage3_hf_total > 0:
            stage3_goals.append(
                f"- MAKE SURE you use {stage3_hf_total} HuggingFace dataset in total to test your models"
            )
        stage4_goals = [
        ...
        self.main_stage_goals: Dict[int, str] = {
            1: _format_goals(stage1_goals),
            2: _format_goals(stage2_goals),
            3: _format_goals(stage3_goals),
            4: _format_goals(stage4_goals),
        }
        # Create initial stage
        self._create_initial_stage()

    def _get_stage_policy(self) -> Dict[str, Any]:
        stages_cfg = getattr(self.cfg.agent, "stages", None)

        def _get_value(key: str, default: Any) -> Any:
            if stages_cfg is None:
                return default
            if isinstance(stages_cfg, dict):
                return stages_cfg.get(key, default)
            return getattr(stages_cfg, key, default)
        ...
        max_stage = _get_int(
            ...
            4,
            min_value=1,
            max_value=max(self.main_stage_dict.keys()),
        )
        return {
            "max_main_stage": max_stage,
            "stage2_min_datasets": _get_int("stage2_min_datasets", 2, min_value=1),
            "stage2_require_improvement_from_base": _get_bool(
                "stage2_require_improvement_from_base", True
            ),
            "stage2_hf_new_datasets": _get_int(
                "stage2_hf_new_datasets", 2, min_value=0
            ),
            "stage3_hf_total_datasets": _get_int(
                "stage3_hf_total_datasets", 3, min_value=0
            ),
        }
    ...
    def _check_stage_completion(self, stage: Stage) -> bool:
        ...
        if stage.stage_number == 2:
            ...
            if not best_node:
                return False, "No best node found"
            if (
                self.stage_policy["stage2_require_improvement_from_base"]
                and best_node == journal.nodes[0]
            ):
                return (
                    False,
    ...
    def _create_next_main_stage(
        ...
        if main_stage_num >= self.stage_policy["max_main_stage"]:
            return None
